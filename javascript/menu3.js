function initSettingsMenu3() {
  ...
  dropdown.addEventListener("change", () => {
    ...
    const applyH2 = confirm("¿Quieres aplicar la fuente seleccionada al Titulo?");
    const applyH3H4 = confirm("¿Quieres aplicar la fuente seleccionada al Texto?");
    const applyButtons = confirm("¿Quieres aplicar la fuente seleccionada a los Botones y Desplegables?");

    if (!applyH2 && !applyH3H4 && !applyButtons) {
    ...
    function updateFonts(currentFonts) {
      if (applyH2) currentFonts.h2 = selectedValue;
      if (applyH3H4) currentFonts.h3h4 = selectedValue;
      if (applyButtons) currentFonts.buttons = selectedValue;
      return currentFonts;
    }
    ...
    const applyInSettings = confirm("¿Quieres aplicar la fuente seleccionads en ajustes?");
    if (applyInSettings) {
...
function initIndexMenu3() {
  const fontIndex = localStorage.getItem("tph_font_index");
  if (fontIndex) {
    applyFont(fontIndex);
  }
...
document.addEventListener("DOMContentLoaded", () => {
  const isSettings = document.getElementById("option3") !== null;
  if (isSettings) {
    initSettingsMenu3();
  } else {
    initIndexMenu3();
